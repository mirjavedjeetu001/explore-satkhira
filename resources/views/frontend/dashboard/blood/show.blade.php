...
<section class="page-header py-4" style="background: linear-gradient(135deg, #dc3545, #a71d2a);">
    <div class="container">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h3 class="text-white mb-0">{{ $donor->name }}</h3>
                <p class="text-white-50 mb-0">ডোনর বিস্তারিত তথ্য</p>
            </div>
            <div class="d-flex gap-2">
                <a href="{{ route('dashboard.blood.edit', $donor->id) }}" class="btn btn-warning"><i class="fas fa-edit me-1"></i>এডিট</a>
                <a href="{{ route('dashboard.blood.index') }}" class="btn btn-light"><i class="fas fa-arrow-left me-1"></i>ফিরে যান</a>
            </div>
        </div>
    </div>
</section>
        ...
        <div class="row g-4">
            <div class="col-md-6">
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white fw-bold"><i class="fas fa-info-circle me-2 text-danger"></i>ডোনর তথ্য</div>
                    <div class="card-body">
                        <table class="table table-borderless mb-0">
                            ...
                            @endif
                            <tr><th>উপজেলা</th><td>{{ $donor->upazila ? ($donor->upazila->name_bn ?? '-') : ($donor->outside_area ? '🌍 ' . $donor->outside_area : '-') }}</td></tr>
                            ...
                            <tr><th>Not Reachable</th><td>
                                ...
                                @if($donor->not_reachable_count > 0)
                                    <form action="{{ route('dashboard.blood.reset-reachable', $donor->id) }}" method="POST" class="d-inline ms-2">
                                        @csrf
            ...
            <div class="col-md-6">
                ...
                <div class="card border-0 shadow-sm">
                    ...
                    <div class="card-body">
                        ...
                        @empty
                            <p class="text-muted text-center py-2">কোনো মন্তব্য নেই।</p>
                        @endforelse
                    </div>
                </div>
            </div>
